...
async function processClippingsFile(plugin: NewslogSyncPlugin, file: File): Promise<void> {
    if (!plugin.settings.username || !plugin.settings.apiKey) {
        new Notice(
            "Username or API Key not configured in plugin settings.",
            5000
        );
        return;
    }
    ...
    try {
        ...
        if (!uploadUrl) {
            new Notice(
                "Failed to get upload URL. Check console for details.",
                7000
            );
            return;
        }
...
async function triggerHighlightsDownload(plugin: NewslogSyncPlugin): Promise<void> {
    if (!plugin.settings.username || !plugin.settings.apiKey) {
        new Notice(
            "Username or API Key not configured in plugin settings.",
            5000
        ...
        for (const s3Key of s3Keys) {
            try {
                ...
                if (existingFile && existingFile instanceof TFile) {
                    ...
                    console.warn(
                        `Path ${filePath} exists but is not a file. Skipping.`
                    );
